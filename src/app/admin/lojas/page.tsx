  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Gerir Lojas</h1>
        <span className="text-sm text-gray-500">{pagination.total} lojas</span>
      </div>
          ...
          {['', 'true', 'false'].map((v) => (
            <button
              ...
              className={cn(
                ...
                verified === v ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              )}
            >
              ...
              <tbody className="divide-y divide-gray-100">
                {stores.map((store) => (
                  <tr key={store.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{store.user?.name || 'N/A'}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{store.province}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{store._count?.products || 0}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{store._count?.reviews || 0}</td>
                    <td className="px-4 py-3">
                      <button
                        ...
                        className={cn(
                          ...
                          store.isVerified ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                        )}
                      >
                ...
                {stores.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500 text-sm">Nenhuma loja encontrada.</td></tr>
                )}
              </tbody>
            </table>
          </div>
