...
import { fetchAdminStores, toggleStoreVerification, deleteStore, type ApiStore } from '@/lib/api-helpers'
...
export default function AdminStoresPage() {
  ...
  const handleToggleVerify = async (storeId: string) => {
    try {
      ...
      toast(t('toggleVerifySuccess'), 'success')
      load()
    } catch {
      toast(t('toggleVerifyError'), 'error')
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500 dark:text-gray-300">{t('storesCount', { count: pagination.total })}</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          {['', 'true', 'false'].map((v) => (
            ...
            >
              {v === '' ? t('filterAll') : v === 'true' ? t('filterVerified') : t('filterUnverified')}
            </button>
          ))}
        </div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3">{t('reviews')}</th>
