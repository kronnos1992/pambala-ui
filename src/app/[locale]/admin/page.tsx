...
import {
  Users, ShoppingCart, Store, Package, DollarSign, Star,
  Eye, ArrowUpRight, ArrowDownRight, UserPlus, Activity
} from 'lucide-react'
...
export default function AdminDashboardPage() {
  ...
  const orderStatusData: OrderStatusPoint[] = stats.recentOrders?.length > 0
? (() => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-950 dark:text-white">{t('title')}</h1>
          <p className="text-sm text-gray-600 dark:text-gray-200 mt-0.5">{t('subtitle')}</p>
        </div>
        <Link href="/admin/pedidos" className="text-sm text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 font-medium flex items-center gap-1 transition-colors">
          {t('viewOrders')} <ArrowUpRight className="h-3.5 w-3.5" />
        </Link>
      </div>
        ...
        <div className="glass-card rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-200">{t('stores')}</p>
            </div>
          </div>
        </div>
        <div className="glass-card rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-200">{t('sellers')}</p>
            </div>
          </div>
        </div>
        <div className="glass-card rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-200">{t('reviews')}</p>
            </div>
          </div>
        </div>
        <div className="glass-card rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-200">{t('verifiedStores')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass-card rounded-2xl border border-gray-200 dark:border-gray-700 p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="font-semibold text-gray-950 dark:text-white">{t('revenueLast30Days')}</h2>
              <p className="text-xs text-gray-600 dark:text-gray-200 mt-0.5">{formatPrice(revenueData.reduce((s, d) => s + d.revenue, 0))} {t('totalLabel')}</p>
            </div>
        ...
        <div className="glass-card rounded-2xl border border-gray-200 dark:border-gray-700 p-5">
          <h2 className="font-semibold text-gray-950 dark:text-white mb-4">{t('ordersByStatus')}</h2>
          {orderStatusData.length > 0 ? (
            <>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {orderStatusData.map((_, index: number) => (
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {orderStatusData.map((item, i: number) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass-card rounded-2xl border border-gray-200 dark:border-gray-700 lg:col-span-2 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700/50">
            <h2 className="font-semibold text-gray-950 dark:text-white">{t('recentOrders')}</h2>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                {stats.recentOrders.length === 0 && (
                  <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500 dark:text-gray-300 text-sm">{t('noOrdersYet')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="space-y-4">
          <h2 className="font-semibold text-gray-950 dark:text-white px-1">{t('quickAccess')}</h2>
          {[
            { href: '/admin/users', icon: Users, label: t('users'), sub: t('usersRegistered', { count: stats.totalUsers }), gradient: 'from-violet-500 to-purple-400', bg: 'bg-violet-100 dark:bg-violet-900/40', text: 'text-violet-700 dark:text-violet-300' },
            { href: '/admin/lojas', icon: Store, label: t('stores'), sub: t('storesRegistered', { count: stats.totalStores }), gradient: 'from-cyan-500 to-blue-400', bg: 'bg-cyan-100 dark:bg-cyan-900/40', text: 'text-cyan-700 dark:text-cyan-300' },
            { href: '/admin/produtos', icon: Package, label: t('products'), sub: t('productsRegistered', { count: stats.totalProducts }), gradient: 'from-amber-500 to-orange-400', bg: 'bg-amber-100 dark:bg-amber-900/40', text: 'text-amber-700 dark:text-amber-300' },
          ].map((link) => (
