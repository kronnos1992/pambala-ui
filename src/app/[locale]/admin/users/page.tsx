...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    try {
      ...
      toast(t('roleUpdateSuccess'), 'success')
      load()
    } catch {
      toast(t('roleUpdateError'), 'error')
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500 dark:text-gray-300">{t('usersCount', { count: pagination.total })}</span>
      </div>
