...
import { ChevronRight, Star, MapPin, Package, MessageCircle, Eye } from 'lucide-react'
...
export default function StoreDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      fetchStoreBySlug(slug, locale),
      fetchStoreProducts(slug, { limit: 20, locale }),
      loadReviews(),
    ])
  ...
  if (!store) {
    return (
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6 text-center py-16">
        ...
        <Link href="/lojas" className="text-emerald-600 hover:text-emerald-700 mt-4 inline-block">{t('backToStores')}</Link>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
      <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-6">
        ...
        <Link href="/lojas" className="hover:text-emerald-600 transition-colors">{tr('stores')}</Link>
        <ChevronRight className="h-3.5 w-3.5" />
        ...
        <div className="flex-1 pb-1">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-1">
            ...
            <div className="flex items-center gap-1 text-sm text-gray-500">
              ...
              <span>{t('productsCount', { count: store._count?.products || 0 })}</span>
            </div>
            <div className="flex items-center gap-1 text-sm text-gray-500">
      ...
      <div className="flex gap-1 border-b border-gray-200 mb-6">
        ...
        >
          {t('productsTab', { count: store._count?.products || 0 })}
        </button>
        <button
        ...
        >
          {t('reviewsTab', { count: totalReviews })}
        </button>
      </div>
      ...
      {activeTab === 'reviews' && (
        <div className="space-y-4 max-w-2xl">
          <ReviewForm storeId={store.id} onSubmitted={loadReviews} />
          <div className="flex items-center gap-4 rounded-xl bg-gray-50 p-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500">{t('reviewsSummary', { count: totalReviews })}</p>
            </div>
          </div>
          {reviews.map((review) => (
