...
export default function ProdutosClient() {
  ...
  React.useEffect(() => {
    ...
    const params: Record<string, unknown> = { page, limit: 12, locale }
    if (q) params.q = q
    ...
    else if (sort === 'best_sellers') params.sort = 'best-sellers'
  ...
  }, [q, category, page, minPrice, maxPrice, condition, sort, province, locale])
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
      <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-6">
        ...
        <span className="text-gray-900 dark:text-white font-medium">{tr('products')}</span>
        {category && (
          <>
            <ChevronRight className="h-3.5 w-3.5" />
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {category || q ? t('resultsFor', { query: category || q }) : t('allProducts')}
          </h1>
        ...
        <div className="flex-1 min-w-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('noProductsTitle')}</h3>
              <p className="mt-1 text-sm text-gray-500">{t('noProductsText')}</p>
            </div>
          )}
        </div>
      </div>
