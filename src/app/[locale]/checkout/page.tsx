...
import { ChevronRight, CreditCard, Banknote, Hash, Truck, Check, Store, type LucideIcon } from 'lucide-react'
...
export default function CheckoutPage() {
  ...
  React.useEffect(() => {
    let active = true
    const load = async () => {
      const storeMap = new Map<string, { storeId: string; storeName: string; items: CartItem[]; subtotal: number }>()
      for (const item of items) {
        const key = item.storeId || 'local'
        if (!storeMap.has(key)) {
          storeMap.set(key, { storeId: key, storeName: item.storeName || t('storeFallback'), items: [], subtotal: 0 })
        }
      ...
      for (const g of storeMap.values()) {
        ...
        if (g.storeId !== 'local') {
          try {
            const store = await fetchStoreBySlug(g.storeId, locale)
  ...
  const validateShipping = () => {
    ...
    if (!form.district.trim()) errs.district = t('errors.districtRequired')
    setErrors(errs)
  ...
  const handleSubmit = async () => {
    ...
    for (const g of groups) {
      ...
      if (!method) {
        toast(t('selectPaymentMethod', { store: g.storeName }), 'error')
        return
      }
    }
    setLoading(true)
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
        <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-6">
          ...
          <span className="text-gray-900 dark:text-white font-medium">{tr('checkout')}</span>
        </nav>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
      <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-6">
        <Link href="/" className="hover:text-emerald-600 transition-colors">{tr('home')}</Link>
        <ChevronRight className="h-3.5 w-3.5" />
        <Link href="/carrinho" className="hover:text-emerald-600 transition-colors">{tr('cart')}</Link>
        <ChevronRight className="h-3.5 w-3.5" />
        <span className="text-gray-900 dark:text-white font-medium">{tr('checkout')}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{t('shippingInfo')}</h2>
          ...
          {groups.map((group) => (
            <div key={group.storeId} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6">
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
                <p className="text-sm font-medium text-gray-900 dark:text-white mb-3">{t('paymentMethod')}</p>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6">
            ...
            <div className="flex justify-between mb-6">
              <span className="text-base font-semibold text-gray-900 dark:text-white">{tc('total')}</span>
            ...
            <Button className="w-full h-12 text-base" onClick={handleSubmit} disabled={loading}>
              {loading ? t('processing') : t('confirmOrders')}
            </Button>
