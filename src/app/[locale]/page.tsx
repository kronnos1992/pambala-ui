...
export default function HomePage() {
  ...
  const steps = [
    {
      icon: Search,
      title: t('steps.find.title'),
      description: t('steps.find.description'),
    },
    {
      icon: CreditCard,
      title: t('steps.buy.title'),
      description: t('steps.buy.description'),
    },
    {
      icon: Smartphone,
  ...
  const testimonials = [
    ...
    {
      name: 'Carlos Silva',
      role: t('testimonials.sellerRole'),
  ...
  const [categories, setCategories] = React.useState<{ id: string; name: string; slug: string; icon?: string }[]>([])

  React.useEffect(() => {
    fetchFeaturedProducts(locale).then(setFeaturedProducts).catch(() => {})
    fetchStores({ limit: 4, locale }).then((data) => setTopStores(data.stores)).catch(() => {})
    import('@/lib/api-helpers').then((m) =>
      m.fetchCategories(locale).then(setCategories).catch(() => {})
    )
  }, [locale])
          ...
          {carouselProducts.length > 0 && (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-white/90">{t('mostVisited')}</h2>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/produtos" className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 font-medium text-sm flex items-center gap-1">
              {t('featuredViewAll')} <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/lojas" className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 font-medium text-sm flex items-center gap-1">
              {t('topStoresViewAll')} <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
          {topStores.length > 0 ? (
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16">
            ...
            >
              ...
              <div>
                <h3 className="font-semibold text-lg">{t('secureTitle')}</h3>
                <p className="text-sm text-emerald-100">{t('secureText')}</p>
              </div>
            </motion.div>
          </div>
        </div>
